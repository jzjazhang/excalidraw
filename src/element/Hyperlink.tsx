...
import { FreedrawIcon, TrashIcon } from "../components/icons";
import { CustomLinkIcon } from "../components/iconsCustom";
import { t } from "../i18n";
import {
  useCallback,
  useEffect,
...
export const actionLink = register({
  ...
  PanelComponent: ({ elements, appState, updateData }) => {
    const selectedElements = getSelectedElements(elements, appState);
    // @ts-ignore
    const { relations, onRelation, customLink, customLinkTitle } =
      appState || {};
    // eslint-disable-next-line
    console.log("是否自定义链接", appState, 'relations', relations, 'onRelation', onRelation, 'customLink', customLink);
    return (
      <ToolButton
        type="button"
        icon={CustomLinkIcon}
        ...
        title={`${
          customLinkTitle
            ? customLinkTitle
            : `${t("labels.link.label")} - ${getShortcutKey("CtrlOrCmd+K")}`
        }`}
        onClick={() =>
          !!customLink ? onRelation(relations) : updateData(null)
        }
        selected={
          !!customLink
            ? selectedElements.length === 1 && relations && relations.length
